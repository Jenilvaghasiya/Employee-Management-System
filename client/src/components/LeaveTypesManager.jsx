...
const initialForm = { name: '', is_paid: false, status: 'Active' };
...
const LeaveTypesManager = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((it) =>
      !q || String(it.name || '').toLowerCase().includes(q) || String(it.status || '').toLowerCase().includes(q)
    );
  }, [items, query]);
  ...
  const fetchAll = async () => {
    try {
      setLoading(true);
      setError('');
      const data = await leaveTypesService.list();
  ...
  return (
    <section className="leave-types-wrap">
          ...
          <div className="modal">
            ...
            <form onSubmit={handleSubmit}>
              <div className="lt-form-grid">
                ...
                <div className="dep-field checkbox-field">
                  <label className="checkbox-label">
                    <input type="checkbox" checked={!!form.is_paid} onChange={(e) => setForm((s) => ({ ...s, is_paid: e.target.checked }))} />
                    <span>Is Paid</span>
                  </label>
                </div>
                <div className="dep-field">
              ...
              {loading ? (
                ...
                filtered.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td>{row.name}</td>
                    <td>
                      <span className={`badge ${row.is_paid ? 'success' : 'muted'}`}>{row.is_paid ? 'Yes' : 'No'}</span>
                    </td>
                    <td>
                ...
                <tr>
                  <td colSpan={5} className="center">No leave types found</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <style>{`
        .leave-types-wrap { display: grid; gap: 20px; }
        ...
        .lt-form-grid { display:grid; grid-template-columns: 1fr 160px 160px; gap:16px }
        .dep-field { display:flex; flex-direction:column; gap:8px }
        .checkbox-field { justify-content:flex-end }
        .checkbox-label { display:flex; align-items:center; gap:8px; font-weight:600; color:#374151 }
        .dep-form-actions, .modal-actions { display:flex; gap:10px; margin-top:4px; }
        ...
        @media (max-width: 860px){ .lt-form-grid { grid-template-columns: 1fr; } }
      `}</style>
    </section>
  );
};
